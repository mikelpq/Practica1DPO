...
public class Location {
    ...
    //OPCIO 2
    public static void findAndInsertLocations(ArrayList<Location> jsonLocations, ArrayList<Location> locations_created, ArrayList<Location> locations_searched, ArrayList<LocationFav> locations_fav){
        int index = 0;
        ...
        for (int i = 0; i < locations_fav.size(); i++) {
            if (locations_fav.get(i).getName().equals(place)){
                while (!stop) {
                    System.out.println(" Aquesta localització està guardada com a preferida\n Desitges eliminar-la?");
                    user_input = sc.nextLine();
                    if (user_input.equals("si") || user_input.equals("no")) {
                        if (user_input.equals("si")) {
                            locations_fav.remove(i);
                            stop = true;
                            System.out.println(place + " s'ha eliminat de les localitzacions preferides");
                        }
                    } else {
    ...
    public static void busTime() throws IOException {
        ...
        //printem
        Printing.showBusLinesFromStop(busTime ,stationsBus, Integer.parseInt(parada));
    }
